...
const EditPasswordValidateScreen = ({navigation}) => {
  ...
  const requestMessageAuthenticate = async () => {
    try {
      const data = await POST_Pnum_Request_EditPassword(pnum.replace(/-/g, ""))
      navigation.replace("EditPasswordValidateCheckScreen", {
        data: {
          pnum: pnum.replace(/-/g, ""),
...
export default EditPasswordValidateScreen
